...
# anarci_path = os.path.split(__file__)
        ...
        if numbered[i] is not None:
            ...
            for j in range(len(numbered[i])):  # Iterate over domains
                ...
                print(
                    "#|%s|%s|%s|%.1f|%d|%d|"
                    % tuple(
                        [alignment_details[i][j][field] for field in ["species", "chain_type", "evalue", "bitscore"]]
...
def _parse_hmmer_query(query, bit_score_threshold=80, hmmer_species=None):
    ...
    """
    hit_table = [["id", "description", "evalue", "bitscore", "bias", "query_start", "query_end"]]
    ...
    if query.hsps:  # We have some hits
        ...
        if hmmer_species:
            hit_correct_species = [
                ...
                if hsp.hit_id.startswith(hmmer_species) and hsp.bitscore >= bit_score_threshold
            ]
