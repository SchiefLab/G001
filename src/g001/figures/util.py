...
def transform_wide_to_long(ptid_df: pd.DataFrame) -> pd.DataFrame:
    # melt wide form to long form
    ptid_df = pd.melt(
        ptid_df,
        id_vars=["PubID", "Site", "Treatment"],
        value_vars=list(filter(lambda x: x[0] == "V", ptid_df.columns.to_list())),
        var_name="Timepoint",
        value_name="Outcome",
    )

    # split out Visit and tissue
    ptid_df = ptid_df.join(
        ptid_df["Timepoint"].apply(
            lambda x: pd.Series(
                {
                    "Visit": x.split()[0],
                    "Tissue": x.split()[-1].replace("(", "").replace(")", ""),
                }
            )
        )
...
def adjust_boxplot(ax: plt.axes) -> plt.subplot:
...
def adjusts_axis(
    ax: plt.axes, ax_index: int, week: int, y_label: str, annotate_args: dict, plot_x_labels: bool, skip_header: bool
):
    # for all panels and axes within panels we can add the same axis
    ...
    if plot_x_labels:
        if len(ax.get_xticklabels()) == 3:
        ...
        elif len(ax.get_xticklabels()) == 2:
            ax.set_xticklabels(
                [r"20 $\mu$g", r"100 $\mu$g"],
                rotation=70,
                ha="right",
                va="center",
                rotation_mode="anchor",
            ...
            raise ValueError("Expected 2 or 3 tick labels")
    else:
        ax.set_xticklabels([])
    ...
    if ax_index > 0:  # and ax_index < 6) or (ax_index == 7):
        ax.set_yticklabels([])
    if ax_index == 0:
        ax.set_ylabel(y_label, size=10, labelpad=10)
    if not skip_header:
        ax.set_title(f"Week {week}")
        if ax_index == 2:
            ax.annotate("Memory B Cells in PBMCs", xy=(0.6, 1.25), **annotate_args)
        if ax_index == 7:
            ax.annotate("GC B Cells in LNs", xy=(0.0, 1.25), **annotate_args)
        if ax_index == 9:
            ax.annotate("Plasmablasts\nin PBMCs", xy=(0.55, 1.25), **annotate_args)
    return ax


def latex_parser(tex_path, table_names: list = None) -> Tuple[str, dict, str]:

    with open(tex_path, "r") as f:
        tex = f.read().split("\n")
    tex = [line.strip() for line in tex]
    ...
    for i, line in enumerate(tex):
        if line.startswith("\\begin{table}"):
            if not header_end_index:
                header_end_index = i
            table_start_index.append(i)
        if line.startswith("\\end{table}"):
            table_end_index.append(i + 1)
    if not header_end_index:
        raise ValueError("No header found")
    header = "\n".join(tex[:header_end_index])

    tables = {}
    for i, (table_start, table_end) in enumerate(zip(table_start_index, table_end_index)):
        ...
        if not caption:
            ...
            caption = table_names[i]
        tables[caption] = "\n".join(table)

    return header, tables, tail
...
def make_tables(outpath: Path, pdf_order: Tuple[str, str], cleanheaders: bool = False, paper_numnber: int = 1) -> None:
    """
    ...
    for caption_prefix, report_table, filename in pdf_order:
        ...
        shutil.copy2(pdf_path, outpath / f"Paper{paper_numnber}/Tables/pdfs/{caption_prefix + '.pdf'}")

    outpath = outpath / f"Paper{paper_numnber}/Tables/"

    merger.write(str(outpath / "all-sup-tables.pdf"))
    merger.close()
    process = subprocess.run(
        ["xelatex", "-output-directory", outpath, outpath / f"SupplementalTablesStagingPaper{paper_numnber}.tex"],
        capture_output=False,
    )
    process.check_returncode()
    (outpath / "all-sup-tables.pdf").unlink()  # A confusing file to keep so just delete it.
